...
appFile = fs.BuildPath(appFolder, "alt_rock_writer.py")

If Not fs.FileExists(appFile) Then
    MsgBox "Alternative Rock Writer could not find alt_rock_writer.py. Keep this launcher in the same folder as the app.", vbExclamation, "Alternative Rock Writer"
    WScript.Quit 1
End If
...
If python = "" Then
    MsgBox "Alternative Rock Writer needs Python 3 with Tkinter. Install Python from python.org with Tcl/Tk support, then open this launcher again.", vbExclamation, "Alternative Rock Writer"
    WScript.Quit 1
End If
...
If Err.Number <> 0 Then
    MsgBox "Alternative Rock Writer could not start: " & launchError, vbExclamation, "Alternative Rock Writer"
    WScript.Quit 1
End If
On Error GoTo 0
