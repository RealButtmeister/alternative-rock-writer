"""Alternative Rock Writer: a Windows desktop notebook for original, raw alternative-rock lyrics."""
...
def default_settings_path():
    folder = Path.home() / 'Documents' / 'Alternative Rock Writer' if getattr(sys, 'frozen', False) else APP_DIR
...
class AlternativeRockWriter(tk.Tk):
    def __init__(self, settings_path=None):
        super().__init__()
        self.title('Alternative Rock Writer · Song JSON')
    ...
    def _styles(self):
        ...
        style.configure('Rock.Horizontal.TProgressbar', background=ACCENT,
    ...
    def button(self, parent, text, command, primary=False):
        return tk.Button(parent, text=text, command=command, relief='flat', bd=0,
                         ...
                         activebackground='#e8ac8e' if primary else '#47413d',
    ...
    def text_field(self, parent, name, height=3):
        ...
        box = tk.Text(frame, height=height, bg=FIELD, fg=INK, insertbackground=ACCENT,
                      selectbackground='#704c3b', relief='flat', bd=0, wrap='word',
    ...
    def _build(self):
        ...
        self.label(left_title, 'ALT ROCK WRITER', 22, bold=True).pack(anchor='w')
        self.label(left_title, 'Raw edges. Heavy hooks. Your words.', 10, MUTED).pack(anchor='w', pady=(3, 0))
        ...
        self.load_song_btn = self.button(header, 'Load song JSON…', self.load_song_structure)
        self.load_song_btn.pack(side='right', padx=(0, 8))
        ...
        self.label(form, 'What is the song about?', bold=True).pack(anchor='w')
        self.text_field(form, 'topic', 2)
        self.label(form, 'Real details + how you actually talk', bold=True).pack(anchor='w')
        self.label(form, 'A room, an object, something you cannot let go.', 9, MUTED).pack(anchor='w')
        self.text_field(form, 'details', 2)
        self.label(form, 'Title or chorus phrase (optional)', bold=True).pack(anchor='w')
        ...
        self.style_var = tk.StringVar(value='90s grunge')
        self.mood_var = tk.StringVar(value='Angry / worn down')
        self.structure_var = tk.StringVar(value='Full song')
        self.phrasing_var = tk.StringVar(value='Short / spacious')
        self.rhyme_var = tk.StringVar(value='Natural / slant rhyme')
        self.dynamics_var = tk.StringVar(value='Quiet / explosive')
        self.imagery_var = tk.StringVar(value='Concrete / uneasy')
        option_rows = [
            [('Sound', self.style_var, ['90s grunge', 'Melodic grunge', 'Heavy / brooding', 'Scrappy garage rock', 'Dark alternative', 'Acoustic / unplugged']),
             ('Mood', self.mood_var, ['Angry / worn down', 'Alienated', 'Numb / detached', 'Restless', 'Bitter / defiant', 'Vulnerable', 'Uneasy / haunted', 'Cathartic'])],
            [('Phrasing', self.phrasing_var, ['Short / spacious', 'Balanced / singable', 'Wordy / conversational']),
             ('Rhymes', self.rhyme_var, ['Natural / slant rhyme', 'Clear end rhymes', 'Loose / unrhymed'])],
            [('Dynamics', self.dynamics_var, ['Quiet / explosive', 'Slow burn', 'Driving throughout', 'Stripped / intimate']),
             ('Imagery', self.imagery_var, ['Concrete / uneasy', 'Direct / blunt', 'Surreal / fragmented'])],
        ]
        ...
        self.label(options, 'Length', 9, MUTED).grid(row=6, column=0, sticky='w')
        self.structure_box = ttk.Combobox(options, textvariable=self.structure_var, state='readonly', width=18,
                     values=['8 lines', '16 lines', '24 lines', '32 lines', '4-line chorus', '8-line chorus', 'Full song', 'Custom arrangement'])
        self.structure_box.grid(row=7, column=0, sticky='ew', padx=(0, 8), pady=(4, 0))
        ...
        tk.Checkbutton(options, text='Swearing is fine', variable=self.explicit_var, bg=PANEL,
                       ...
                       highlightthickness=0).grid(row=7, column=1, sticky='w')
        self.arrange_btn = self.button(options, 'Song structure…', self.structure_dialog)
        self.arrange_btn.grid(row=8, column=0, columnspan=2, sticky='ew', pady=(10, 0))
        self.clear_song_btn = self.button(options, 'Clear imported song map', self.clear_song_structure)
        self.clear_song_btn.grid(row=9, column=0, columnspan=2, sticky='ew', pady=(6, 0))
        ...
        self.status_var = tk.StringVar(value='Ready. Start with an image that gets under your skin.')
        self.status_label = tk.Label(footer, textvariable=self.status_var, bg=BG, fg=MUTED, anchor='w')
        self.status_label.pack(fill='x')
        self.progressbar = ttk.Progressbar(footer, mode='indeterminate', style='Rock.Horizontal.TProgressbar')
    ...
    def structure_dialog(self):
        ...
        dialog.title('Alternative Rock Writer — Song structure')
        ...
        def preset(entries):
            sections[:] = entries
            refresh(0)
        self.button(presets, 'Verse + chorus', lambda: preset([('Verse 1', 8), ('Chorus', 4)])).pack(side='left')
        self.button(presets, 'Full song', lambda: preset(list(DEFAULT_SONG_STRUCTURE))).pack(side='left', padx=8)
        self.button(presets, 'One verse', lambda: preset([('Verse', 8)])).pack(side='left')
        listing = tk.Frame(dialog, bg=PANEL)
        listing.pack(fill='both', expand=True)
        song_list = tk.Listbox(listing, bg=FIELD, fg=INK, selectbackground='#704c3b', height=6,
        ...
        self.label(edits, 'Lines', 9, MUTED).grid(row=0, column=1, sticky='w', padx=(10, 0))
        name_var = tk.StringVar(value='Verse')
        bars_var = tk.StringVar(value='8')
        ttk.Combobox(edits, textvariable=name_var, values=['Intro', 'Verse 1', 'Verse 2', 'Pre-chorus', 'Chorus', 'Post-chorus', 'Bridge', 'Final chorus', 'Outro']).grid(row=1, column=0, sticky='ew', pady=(5, 0))
        ...
        def refresh(index=None):
            ...
            for n, (name, bars) in enumerate(sections, 1):
                song_list.insert('end', f'  {n:02d}    {name}    /    {bars} lines')
        ...
        self.label(dialog, 'Count sung phrases, not musical bars. Up to 12 sections / 160 lines.', 9, MUTED).pack(anchor='w')
        self.label(dialog, 'Repeat choruses freely. Generated section order and line counts are checked.', 9, MUTED).pack(anchor='w', pady=(3, 0))
        def apply():
            index = selected()
            if index is not None:
                entry = get_entry()
    ...
    def song_map_dialog(self):
        ...
        self.label(dialog, 'Keep the music map. Set how many sung lines each section needs; instrumental sections use 0.', 9, MUTED, wraplength=650).pack(anchor='w', pady=(0, 10))
    ...
    def connection_dialog(self):
        if self.busy:
            return
        dialog = tk.Toplevel(self)
        dialog.title('Alternative Rock Writer — Connection')
        dialog.configure(bg=PANEL, padx=24, pady=22)
        dialog.transient(self)
        ...
        self.label(dialog, 'Word checks are strict. Sing the draft aloud and make it yours.', 9, MUTED).pack(anchor='w')
    ...
    def _update_gate(self):
        ...
        if not text:
            ...
            label = 'Word rules passed  /  Read it aloud and make it yours.'
        ...
        try:
            while True:
                ...
                if kind == 'progress':
                    ...
                    self._set_busy(False)
                    self.status_var.set(payload)
                    messagebox.showerror('Could not finish these lyrics', payload, parent=self)
        except queue.Empty:
            pass
        self.after(120, self._poll)
    ...
    def _copy_selection(self, event=None):
        ...
        if self._gate_problem(text, check_structure=False):
            self.status_var.set('Copy the whole song to include every required good word.')
            return 'break'
        self.clipboard_clear()
        self.clipboard_append(text)
        return 'break'
    ...
    def save_lyrics(self):
        ...
        path = filedialog.asksaveasfilename(parent=self, title='Save your lyrics',
                                          initialdir=str(self.settings_path.parent), initialfile='My alternative rock song.txt',
...
if __name__ == '__main__':
    if os.name == 'nt':
        try:
            ...
            pass
    AlternativeRockWriter().mainloop()
